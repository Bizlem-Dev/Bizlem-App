...
ArrayList<Node> pb = null;
		...
		if (!searchText.trim().equals("")) {
			try {
				...
				String querryStr = "select [name] from [nt:base] where (contains('name','*"+searchText +"*'))  and ISDESCENDANTNODE('/content/user/')";
			...
			BufferedReader rd = new BufferedReader(new InputStreamReader(is, Charset.forName("UTF-8")));
		      String jsonText = this.readAll(rd);
		       ...
		       return "0";
				...
				// TODO Auto-generated catch block
				return "mal1"+e.getMessage();
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				return "mal2"+e.getMessage();
			}finally {
		      try {
				is.close();
			} catch (IOException e) {
